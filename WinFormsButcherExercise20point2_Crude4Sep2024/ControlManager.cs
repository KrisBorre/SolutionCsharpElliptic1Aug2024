...
namespace WinFormsButcherExercise20point2_Crude4Sep2024
{
    internal class ControlManager
    {
        ...
        public ControlManager(int width, int height)
        {
            ...
            */
            ...
            var butcher = new DifferentialEquationsButcherExercise20point2_2Sep2024<double>();
            int numberOfFirstOrderEquations = butcher.NumberOfFirstOrderEquations;
            ISolver26feb2024<double> solver1 = new DifferentialEquationsSolver26feb2024<double>(butcher, Method.Euler | Method.Sophisticated);
            ISolver26feb2024<double> solver2 = new DifferentialEquationsSolver26feb2024<double>(butcher, Method.Euler | Method.Crude);

            double interval = 1; // Math.PI;

            double initial_y2 = 0.5;
            double C = 1.0 / initial_y2; // 2
            double y2_initial = y2_exact_function(0, C); // 0.5

            ConditionInitial26feb2024<double> ic = new ConditionInitial26feb2024<double>(0, 0, y2_exact_function(0, C));
            //ic.X = 0;

            ulong number_of_steps = 20;
            ...
            for (int k = 0; k < kmax; k++)
            {
                ...
                double y2_interval_exact = y2_exact_function(interval, C);
                ...
                double error_sophisticated = sqrt(Math.Pow((y2_interval_exact - y_sophisticated[1]), 2));
                Console.WriteLine("error_sophisticated = " + error_sophisticated);

                double error_crude = sqrt(Math.Pow((y2_interval_exact - y_crude[1]), 2));
